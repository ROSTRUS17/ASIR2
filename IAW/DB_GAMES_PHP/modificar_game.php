...
<div class="container">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_update = "UPDATE GAMES SET NOMBRE='$nuevo_nombre', GENERO='$nuevo_genero' WHERE ID=$id_update";
        
        if ($conn->query($sql_update) === TRUE) {
            echo "<p style='color:#fff; background:#1a5c00; padding:10px; text-align:center;'> >> JUEGO ACTUALIZADO << </p>";
            $row['NOMBRE'] = $nuevo_nombre;
            $row['GENERO'] = $nuevo_genero;
        } else {
    ...
    <form method="post" action="">
        ...
        <label>> GENERO_JUEGO:</label>
        <input type="text" name="genero" value="<?php echo $row['GENERO']; ?>" required>
        
        <input type="submit" value="ACTUALIZAR_JUEGO">
    </form>
</div>
